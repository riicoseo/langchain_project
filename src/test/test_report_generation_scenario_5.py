...
def main():
    print("\n" + "="*100)
    print("  시나리오 5: 단일 주식 - 보고서 + 차트 + 저장 (.pdf)")
    print("="*100 + "\n")
    ...
    if report_result['saved_path'] and os.path.exists(report_result['saved_path']):
        if report_result['saved_path'].endswith('.pdf'):
            ...
            print(f"⚠️  WARNING: PDF가 아닌 형식으로 저장됨: {report_result['saved_path']}")
    else:
        print(f"❌ FAIL: 파일이 저장되지 않았습니다")

    print(f"\n{'✅ 시나리오 5 통과' if len(report_result['charts']) > 0 and report_result['saved_path'] and report_result['saved_path'].endswith('.pdf') else '❌ 시나리오 5 실패'}\n")
