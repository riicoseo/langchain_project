"""
시나리오 10: 다중 주식 - 비교 보고서 + 차트 + 저장 (.pdf)
"""
...
def main():
    print("\n" + "="*100)
    print("  시나리오 10: 다중 주식 - 비교 보고서 + 차트 + 저장 (.pdf)")
    print("="*100 + "\n")
    ...
    print("STEP 2: Report Generator 실행 중 (차트 + 저장 .pdf)...")
    generator = ReportGenerator()  # Config.LLM_MODEL 사용
    report_result = generator.generate_report(
        "애플과 마이크로소프트 비교 분석 보고서를 차트와 함께 PDF로 저장해주세요",
        analysis_result
    )
    ...
    if report_result['saved_path'] and os.path.exists(report_result['saved_path']):
        if report_result['saved_path'].endswith('.pdf'):
            size = os.path.getsize(report_result['saved_path'])
            print(f"✅ PASS: .pdf 파일 저장 - {report_result['saved_path']} ({size} bytes)")
        else:
            print(f"⚠️  WARNING: .pdf가 아닌 형식")
    else:
        print(f"❌ FAIL: 파일이 저장되지 않았습니다")

    print(f"\n{'✅ 시나리오 10 통과' if len(report_result['charts']) > 0 and report_result['saved_path'] and report_result['saved_path'].endswith('.pdf') else '❌ 시나리오 10 실패'}\n")
